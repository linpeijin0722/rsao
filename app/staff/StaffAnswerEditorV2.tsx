...
export default function StaffAnswerEditorV2({ value, profiles, change, close, save }: any) {
  ...
  return <div className="modalBackdrop returnedEditBackdrop"><div className="modal staffFrontEditor staffAnswerEditor">
    ...
    <div className="staffFrontFields staffExactAnswerFields">
      ...
      {personalLove && <section className="staffAnswerSection">{area("目前感情狀態", "love_status", "例如：單身多久、剛分手沉澱中、空窗期較長等")}{area("目前的社交與生活型態", "social_lifestyle", "例如：生活圈固定不太出門、正積極使用交友軟體或參加活動等")}{area("本次最想瞭解的方向", "love_direction", "例如：未來一年內的桃花時機、適合自己的伴侶類型、或個人感情盲點等")}</section>}
      ...
      {infant && <section className="staffAnswerSection">{(extra.pregnancy_losses || []).map((loss: any, i: number) => <div className="staffNestedFields" key={i}><label>國曆流產時間<input type="date" value={text(loss.date)} onChange={(e) => { const next=[...(extra.pregnancy_losses||[])]; next[i]={...loss,date:e.target.value};setExtra("pregnancy_losses",next)}}/></label><label>農曆流產時間<input disabled value={text(loss.lunar)}/></label><label>所選時間為<select value={text(loss.accuracy)} onChange={(e) => { const next=[...(extra.pregnancy_losses||[])]; next[i]={...loss,accuracy:e.target.value};setExtra("pregnancy_losses",next)}}><option>準確時間</option><option>大約時間</option></select></label><label>備註<textarea value={text(loss.notes)} onChange={(e) => { const next=[...(extra.pregnancy_losses||[])]; next[i]={...loss,notes:e.target.value};setExtra("pregnancy_losses",next)}}/></label></div>)}{area("目前的心理或生活狀況", "current_condition")}{area("過去是否曾處理過", "previous_handling")}{area("這次最希望獲得的協助", "assistance_goal")}</section>}
      ...
      {!noGenericQuestions && <section className="staffAnswerSection"><h4>想詢問的問題</h4>{(value.questions?.length ? value.questions : [""]).map((q: string, i: number) => <label key={i}>輸入你想問的問題，也可點下方新增問題<textarea value={q || ""} onChange={(e) => setQuestion(i, e.target.value)}/></label>)}</section>}
    </div>
...
function TargetQuestions({ primary, profile, kind, questions, update }: any) {
  ...
  return <section className="staffAnswerSection targetQuestionSection"><h4>關於「{primary?.name || "諮詢者"}」與「{profile?.name || "這位對象"}」的{kind}，我想問</h4>{list.map((question, index) => <label key={index}>輸入你想問的問題，也可點下方新增問題<textarea value={question} onChange={(e) => { const next=[...list];next[index]=e.target.value;update(next)}}/></label>)}</section>;
}
