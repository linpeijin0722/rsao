...
export default function Staff() {
  ...
  return (
    <main className={`staffPage returned-edit-${returnedEditMode}`}>
      ...
      {dataView.length > 0 && (
        <div className="modalBackdrop" onClick={() => setDataView([])}>
          ...
          >
            ...
            {dataViewMode!=="menu"&&(dataViewMode==="user"?dataView.filter((p:any,i:number,all:any[])=>all.findIndex(x=>x.id===p.id)===i):dataView).map((p: any, i: number) => (
              <article key={p.id || i}>
                <b>
                ...
                {p.notes && <p>{p.notes}</p>}{dataViewMode!=="view"&&<button onClick={()=>dataViewMode==="user"?setProfileEditor({...p}):setAnswerEditor({...p,targetProfileId:p.id,questions:[0,1,2].map(i=>p.questions?.[i]||"")})}>{dataViewMode==="user"?"修改這位人物資料":"修改這個項目的問事資料"}</button>}
              </article>
            ))}
      ...
      {answerEditor&&<StaffAnswerEditor value={answerEditor} profiles={dataView.filter((p:any,i:number,all:any[])=>all.findIndex(x=>x.id===p.id)===i)} change={setAnswerEditor} close={()=>setAnswerEditor(null)} save={saveAnswerEditor}/>} 
