...
export default function Staff() {
  ...
  async function saveAnswerEditor() {
    const r=await fetch("/api/staff/bookings",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"update_answer",answerId:answerEditor.answerId,profileId:answerEditor.targetProfileId,profileIds:answerEditor.targetProfileIds,questions:answerEditor.questions})}),j=await r.json();if(!r.ok)return alert(j.error||"儲存失敗");alert("問事資料已更新");setAnswerEditor(null);setDataView([]);await load();
  }
      ...
      {dataView.length > 0 && (
        <div className="modalBackdrop" onClick={() => setDataView([])}>
          ...
          >
            ...
            <h2>已回傳的諮詢者資料</h2>
            {dataViewMode==="menu"&&<div className="returnedActionMenu"><button onClick={()=>setDataViewMode("view")}>查看內容</button><button onClick={()=>setDataViewMode("answers")}>修改問事資料</button></div>}
...
function StaffAnswerEditorV2({value,profiles,change,close,save}:any){const ids:Array<string>=value.targetProfileIds?.length?value.targetProfileIds:[value.targetProfileId].filter(Boolean);return <div className="modalBackdrop returnedEditBackdrop"><div className="modal staffFrontEditor staffAnswerEditor"><button className="staffModalClose" onClick={close}>×</button><h2>修改問事資料</h2><div className="staffAnswerPeople"><b>請選擇諮詢者</b>{ids.map((id:string,index:number)=><label key={index}>第 {index+1} 位<select value={id} onChange={e=>{const next=ids.map((v,n)=>n===index?e.target.value:v);change({...value,targetProfileId:next[0]||"",targetProfileIds:next})}}><option value="">請選擇資料</option>{profiles.map((p:any)=><option key={p.id} value={p.id} disabled={p.id!==id&&ids.includes(p.id)}>{p.name}（{p.relationship_detail||p.relationship}）</option>)}</select></label>)}</div><h3 className="staffAnswerItem">{value.item_title}{value.sub_items?.length?`－${value.sub_items.join("、")}`:""}</h3><div className="staffFrontFields">{value.questions.map((q:string,i:number)=><label key={i}>問題 {i+1}<textarea value={q} onChange={e=>change({...value,questions:value.questions.map((v:string,n:number)=>n===i?e.target.value:v)})}/></label>)}</div><div className="returnedEditActions"><button onClick={()=>void save()} disabled={!ids.length||ids.some(id=>!id)||new Set(ids).size!==ids.length}>儲存問事資料</button><button className="cancel" onClick={close}>取消</button></div></div></div>}
