...
export default function Staff() {
  ...
  async function saveAnswerEditor() {
    const r=await fetch("/api/staff/bookings",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"update_answer",answerId:answerEditor.answerId,profileId:answerEditor.targetProfileId,questions:answerEditor.questions})}),j=await r.json();if(!r.ok)return alert(j.error||"儲存失敗");alert("問事資料已更新");setAnswerEditor(null);setDataView([]);await load();
  }
      ...
      {dataView.length > 0 && (
        <div className="modalBackdrop" onClick={() => setDataView([])}>
          ...
          >
            ...
            {dataViewMode==="user"&&<div className="staffProfileTags">{dataView.filter((p:any,i:number,all:any[])=>all.findIndex(x=>x.id===p.id)===i).map((p:any)=><button key={p.id} className="staffProfileTag" onClick={()=>setProfileEditor({...p})}>{p.profile_type==="pet"&&p.photo_data&&<img src={p.photo_data} alt=""/>}<span><b>{p.name}</b><small>{p.relationship}{p.relationship_detail?`・${p.relationship_detail}`:""}</small></span></button>)}</div>}
            {dataViewMode==="answers"&&<div className="staffAnswerCards">{dataView.filter((p:any,i:number,all:any[])=>all.findIndex(x=>x.answerId===p.answerId)===i).map((p:any)=><article key={p.answerId}><div className="answerProfileTags">{dataView.filter((person:any)=>person.answerId===p.answerId).filter((person:any,i:number,all:any[])=>all.findIndex(x=>x.id===person.id)===i).map((person:any)=><button key={person.id} onClick={()=>setProfileEditor({...person})}>{person.profile_type==="pet"&&person.photo_data&&<img src={person.photo_data} alt=""/>}<span><b>{person.name}</b><small>{person.relationship_detail||person.relationship}</small></span></button>)}</div><h3>{p.item_title}{p.sub_items?.length?`－${p.sub_items.join("、")}`:""}</h3><div className="answerReadContent">{p.questions?.filter(Boolean).length?p.questions.filter(Boolean).map((q:string,n:number)=><p key={n}>問題 {n+1}：{q}</p>):<p>未填寫問題</p>}</div><button className="editAnswerButton" onClick={()=>setAnswerEditor({...p,targetProfileId:p.id,questions:[0,1,2].map(i=>p.questions?.[i]||"")})}>修改該項目問事資料</button></article>)}</div>}
...
function StaffAnswerEditorV2({value,profiles,change,close,save}:any){const [choose,setChoose]=useState(false),selected=profiles.find((p:any)=>p.id===value.targetProfileId);return <div className="modalBackdrop returnedEditBackdrop"><div className="modal staffFrontEditor staffAnswerEditor"><button className="staffModalClose" onClick={close}>×</button><h2>修改問事資料</h2><div className="selectedConsultant"><span>諮詢者</span><button onClick={()=>setChoose(!choose)}>{selected?.profile_type==="pet"&&selected.photo_data&&<img src={selected.photo_data} alt=""/>}<b>{selected?.name||"請選擇"}</b><small>{selected?.relationship_detail||selected?.relationship||""}</small></button>{selected&&<div className="consultantPreview"><span>{selected.gender||""}</span><span>{selected.birth_date||""}</span><span>{selected.birth_shichen||""}</span></div>}</div>{choose&&<div className="answerPersonPicker">{profiles.map((p:any)=><button className={p.id===value.targetProfileId?"selected":""} key={p.id} onClick={()=>{change({...value,targetProfileId:p.id});setChoose(false)}}>{p.profile_type==="pet"&&p.photo_data&&<img src={p.photo_data} alt=""/>}<span><b>{p.name}</b><small>{p.relationship_detail||p.relationship}</small></span></button>)}</div>}<h3 className="staffAnswerItem">{value.item_title}{value.sub_items?.length?`－${value.sub_items.join("、")}`:""}</h3><div className="staffFrontFields">{value.questions.map((q:string,i:number)=><label key={i}>問題 {i+1}<textarea value={q} onChange={e=>change({...value,questions:value.questions.map((v:string,n:number)=>n===i?e.target.value:v)})}/></label>)}</div><div className="returnedEditActions"><button onClick={()=>void save()}>儲存問事資料</button><button className="cancel" onClick={close}>取消</button></div></div></div>}
