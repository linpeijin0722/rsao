...
const submissionTime = (value:string) => {
  const parts=new Intl.DateTimeFormat("zh-TW",{timeZone:"Asia/Taipei",year:"numeric",month:"numeric",day:"numeric",hour:"numeric",minute:"2-digit",hour12:true}).formatToParts(new Date(value));
  const part=(type:string)=>parts.find(entry=>entry.type===type)?.value||"";
  const hour=Number(new Intl.DateTimeFormat("en-US",{timeZone:"Asia/Taipei",hour:"numeric",hourCycle:"h23"}).format(new Date(value)));
  const period=hour===12?"中午":hour>=13?"下午":hour>=5?"上午":"凌晨";
  return `${part("year")}/${part("month")}/${part("day")} ${period}${hour===0?12:hour>12?hour-12:hour}:${part("minute")}`;
};
...
export default function Staff() {
  const [password, setPassword] = useState(""),
    ...
    [selectedSubmission,setSelectedSubmission]=useState<any>(null),
    ...
    [documentVersionPicker,setDocumentVersionPicker]=useState<any>(null),
  ...
  async function selectBankAccount(account:any){
    if(account.id===paymentSettings?.bank_account_key)return;
  ...
  async function generateDocument(x:any,createMode:"replace"|"new"="replace",confirmedTeacherEdit=false){
    ...
    try{
      const response=await staffPost({action:"generate_document",bookingNo:x.booking_no,force:exists,createMode,submissionId:x._submissionId}),result=await response.json();
  ...
  function openReturnedData(booking:any){
    ...
    setSelectedSubmission(null);setDataViewMode("menu");setDataView(merged);setDataBooking(booking);
  }
  ...
  async function saveProfileEditor() {
    if(!confirm("是否確認要修改用戶資料並儲存？"))return;
    const action=profileEditor.submissionId?"update_submission_profile":"update_consultation_profile";
    const r=await fetch("/api/staff/bookings",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action,submissionId:profileEditor.submissionId,profileId:profileEditor.id,profile:profileEditor})}),j=await r.json();if(!r.ok)return alert(j.error||"儲存失敗");alert("用戶資料已更新");setProfileEditor(null);setSelectedSubmission(null);setDataView([]);await load();
  }
  async function saveAnswerEditor() {
    if(!confirm("是否確認要修改問事資料並儲存？"))return;
    const action=answerEditor.submissionId?"update_submission_answer":"update_answer";
    const r=await fetch("/api/staff/bookings",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action,submissionId:answerEditor.submissionId,answerId:answerEditor.answerId,profileId:answerEditor.targetProfileId,profileIds:answerEditor.targetProfileIds,questions:answerEditor.questions,extraData:answerEditor.extra_data||{}})}),j=await r.json();if(!r.ok)return alert(j.error||"儲存失敗");alert("問事資料已更新");setAnswerEditor(null);setSelectedSubmission(null);setDataView([]);await load();
  }
  const filtered = useMemo(() => {
    ...
    daily = video.filter((x) => {
      ...
      const slotTime = new Date(x.slot_start).getTime();
       if (showUpcomingVideo) return statusKey(x) === "paid" && slotTime >= Date.now();
      if (showNewVideo) {
         const twoWeeksAgo = Date.now() - 14 * 24 * 60 * 60 * 1000;
         const createdTime = new Date(x.created_at).getTime();
         return createdTime >= twoWeeksAgo && createdTime <= Date.now();
      }
  ...
  return (
    <main className={`staffPage returned-edit-${returnedEditMode}`}>
      ...
      {lineContact&&<div className="modalBackdrop priorityModal" onClick={()=>setLineContact(null)}><div className="modal lineContactModal" onClick={e=>e.stopPropagation()}>
        ...
        {lineContact._booking?.consultation_methods?.code==="video"&&lineContact._booking?.payment_status==="paid"&&<button className="videoReminderPreset" onClick={()=>setVideoReminderConfirm(true)}><span aria-hidden="true">⏰</span><b>提醒用戶視訊時間</b><small>傳送預約日期、時間與視訊操作說明</small></button>}
      ...
      {answerEditor&&<StaffAnswerEditorV2 value={answerEditor} profiles={answerEditor.availableProfiles||uniqueProfilesById(dataView)} change={setAnswerEditor} close={()=>setAnswerEditor(null)} save={saveAnswerEditor}/>}
