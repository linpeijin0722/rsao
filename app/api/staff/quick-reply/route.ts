...
export async function POST(request:NextRequest){
  ...
  try{
    ...
    if(body.mode==="compose"||body.mode==="compose_section"){
      ...
      const optionCodes=new Set(valid.map(selection=>selection.optionCode)),combineLotusIngot=optionCodes.has("assistance_lotus")&&optionCodes.has("assistance_ingot"),elementRows=valid.filter(selection=>selection.optionCode.startsWith("element_")),deityRows=valid.filter(selection=>selection.optionCode.startsWith("deity_")&&selection.optionCode!=="deity_custom"),excluded=(selection:any)=>selection.optionCode==="location"||selection.optionCode.startsWith("element_")||selection.optionCode.startsWith("deity_")||(combineLotusIngot&&["assistance_lotus","assistance_ingot"].includes(selection.optionCode)),chosen=valid.filter(selection=>!excluded(selection)).map(selection=>pick((phrases||[]).filter((entry:any)=>entry.option_id===selection.optionId),previous)).filter(Boolean) as any[],locationSelected=valid.some(selection=>selection.optionCode==="location"),customLocation=clean(body.customLocation),locationSubject=clean(body.locationSubject)||"祂",reincarnatedAs=clean(body.reincarnatedAs),locationMode=clean(body.locationMode),locationSentence=locationSelected?(locationMode==="reincarnated"&&reincarnatedAs?`目前已投胎成一個${reincarnatedAs}，`:customLocation?`${locationSubject}現在在${customLocation}。`:""):"";
      const subject=clean(body.locationSubject)||"祂",pronoun=clean(body.genderPronoun)||"祂",render=(value:any)=>String(value||"").replaceAll("{subject}",subject).replaceAll("{pronoun}",pronoun).trim(),combinedHelp=combineLotusIngot?[`${subject}目前還需要一些助力，有空的時候可以再多準備一些蓮花跟銀色元寶，燒化後迴向給${subject}，能補足目前比較欠缺的助力。`,`有空的時候可以幫${subject}一起準備蓮花跟銀色元寶，燒化後再迴向給${subject}，讓後面的路走得比較順。`,`蓮花跟銀色元寶可以一起多準備一些，有空的時候燒化再迴向給${subject}，對目前的狀況會有幫助。`][Math.floor(Math.random()*3)]:"";
