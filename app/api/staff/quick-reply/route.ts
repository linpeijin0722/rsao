...
async function context(bookingNo:string,requestedDocumentId=""){
  ...
  const {data:booking,error}=await db.from("bookings").select("id,booking_no,customers(line_display_name,full_name),booking_details(id,created_at,item_title,google_document_id,google_document_url,booking_consultation_answers(questions))").eq("booking_no",bookingNo).single();
  ...
  let questionSlots=await getQuickReplyQuestionSlots(documentDetail.google_document_id);
  if(!questionSlots.length){
    ...
    questionSlots=fallback.map((question:string,slotIndex:number)=>({slotIndex,questionNumber:slotIndex+1,question,answer:""}));
  }
  ...
  const recommendedByQuestion=Object.fromEntries(questionSlots.map(slot=>[String(slot.slotIndex),normalizedTopics.filter((topic:any)=>asArray(topic.keywords).some((keyword:string)=>slot.question.toLocaleLowerCase("zh-TW").includes(String(keyword).toLocaleLowerCase("zh-TW")))).map((topic:any)=>topic.code)]));
  ...
  const questionReplies=Object.fromEntries(questionSlots.map(slot=>{const existing=savedReplies[String(slot.slotIndex)]||{};return [String(slot.slotIndex),{selections:existing.selections||{},phraseIds:existing.phraseIds||[],answer:existing.answer||slot.answer||""}]}));
...
export async function POST(request:NextRequest){
  ...
  try{
    ...
    for(const slot of data.questionSlots){const row=incoming[String(slot.slotIndex)]||{},answer=normalizeConsultationReturnText(String(row.answer||""));if(!answer)continue;questionReplies[String(slot.slotIndex)]={selections:row.selections||{},phraseIds:asArray(row.phraseIds).map(String),answer};answers[String(slot.slotIndex)]=answer}
