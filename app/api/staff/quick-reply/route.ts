...
const dateResultBuiltInRows = [
  ...
  ["date_notice_traffic", "避開生肖相沖者在場", "吉時進行重要儀式時，生肖相沖者要先迴避，不要站在主位旁。"],
  ["date_notice_simple", "儀式簡單莊重", "儀式保持簡單莊重，不用安排得太複雜。"],
  ["date_notice_documents", "吉時一到立即進行", "吉時一到就要開始主要儀式，不可拖延，也不要錯過時辰。"],
  ["date_notice_weather", "當日忌爭吵與見血", "當日要避開爭吵、哭鬧與見血，保持氣場清淨，才不會沖散吉氣。"],
  ["date_notice_people", "主事者不可臨時更換", "主事者與主要參與者不要臨時更換，避免人不對位而破壞原定吉局。"],
...
const pastLifeBuiltInRows = [
  ...
  ...[
    ["direct", "熱情直率", "個性熱情直率，待人真誠，也願意把想法說清楚。"],
    ["rational", "冷靜理性", "遇到事情能冷靜分析，重視道理與實際結果。"],
    ["smart", "聰明敏銳", "反應聰明敏銳，能很快看出事情的重點。"],
    ["careful", "心思細膩", "心思細膩，能注意到別人容易忽略的細節。"],
    ["confident", "有自信", "做事情有自信，面對問題時不容易退縮。"],
    ["leader", "有領導力", "有領導能力，遇到事情能主動承擔與安排。"],
    ["kind", "本性善良", "本性善良，願意體諒別人，也有助人的心。"],
    ["responsible", "有責任感", "對承諾與責任很重視，交代的事情會盡力完成。"],
    ["observant", "善於觀察", "善於觀察環境與他人的反應，也懂得察言觀色。"],
    ["creative", "有創造力", "有創造能力，常能想到不同的做法與方向。"],
  ].flatMap(([code,label,content])=>[
    [`past_consultant_pro_${code}`,label,`{selfName}${content}`],
    [`past_target_pro_${code}`,label,`{partnerName}${content}`],
  ]),
  ...[
    ["proud", "好面子", "比較好面子，有時候會因為在意形象而不願意退讓。"],
    ["subjective", "較主觀", "處理事情比較主觀，容易先照自己的想法判斷。"],
    ["suspicious", "容易多疑", "遇到事情容易多疑，也會反覆猜測別人的想法。"],
    ["calculating", "過度計較", "有時候會過度計較，容易把小事情放在心上。"],
    ["stubborn", "容易鑽牛角尖", "遇到不順時容易鑽牛角尖，需要有人適時提醒。"],
    ["impatient", "個性較急", "個性比較急，事情沒有照預期時容易不耐煩。"],
    ["sensitive", "情緒敏感", "情緒比較敏感，別人的語氣與態度容易影響心情。"],
    ["controlling", "喜歡掌控", "做事情喜歡掌控全局，不容易放心交給別人。"],
    ["conceited", "容易自負", "有時候會過度相信自己的判斷，容易顯得自負。"],
    ["negative", "容易負面思考", "遇到壓力時容易往負面的方向思考，讓自己更煩心。"],
  ].flatMap(([code,label,content])=>[
    [`past_consultant_con_${code}`,label,`{selfName}${content}`],
    [`past_target_con_${code}`,label,`{partnerName}${content}`],
  ]),
  ...
  ["past_relationship_listen", "先聽完再回應", "溝通時要先把對方的話聽完，再說自己的想法，才不會一直誤會。"],
  ["past_relationship_calm", "情緒平穩再談", "有爭執時先讓情緒平穩，再談真正的問題，效果會比較好。"],
  ["past_relationship_no_cold", "避免冷戰", "兩個人有問題不要用冷戰處理，拖得越久越容易累積不滿。"],
  ["past_relationship_respect", "尊重彼此決定", "重要事情可以討論，但也要尊重彼此最後的決定。"],
  ["past_relationship_responsibility", "責任分配清楚", "生活與關係中的責任要分配清楚，才不會總覺得只有自己在付出。"],
  ["past_relationship_appreciate", "表達感謝", "平常要把感謝與在意說出來，不要覺得對方一定知道。"],
  ["past_relationship_trust", "建立信任", "遇到疑問要直接確認，不要只靠猜測，才能慢慢建立信任。"],
  ["past_relationship_goal", "建立共同目標", "兩個人可以建立共同目標，關係會更有方向，也比較能互相扶持。"],
  ["past_relationship_time", "安排相處時間", "再忙也要固定安排相處時間，關係才不會因為疏忽而變淡。"],
  ["past_relationship_decide", "重要事情共同決定", "遇到重要事情要一起討論與決定，不要讓其中一個人總是被動接受。"],
] as const;
