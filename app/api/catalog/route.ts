  ...
  try {
    ...
    const [
      ...
      db
        ...
        .select(
          "id,code,title,description,duration_minutes,base_price,monthly_limit,reply_days,sort_order",
        )
        .eq("is_active", true)
        .order("sort_order"),
      db
        .from("booking_items")
        .select(
          "id,code,title,description,price,allow_quantity,option_mode,sort_order",
        )
        .eq("is_active", true)
        .order("sort_order"),
      db
