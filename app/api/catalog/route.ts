...
export async function GET() {
  try {
    ...
    const [{ data: methods, error: methodError }, { data: items, error: itemError }, { data: subItems, error: subError }] = await Promise.all([
      ...
      db.from("booking_items").select("id,title,description,price,allow_quantity,option_mode,sort_order").eq("is_active", true).order("sort_order"),
      db.from("sub_items").select("id,item_id,title,description,price,is_default,sort_order").eq("is_active", true).order("sort_order"),
    ]);
