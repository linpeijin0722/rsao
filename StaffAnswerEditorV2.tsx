    ...
    <div className="staffFrontFields staffExactAnswerFields">
      ...
      {love && targetIds.map((id) => {
        ...
        return <section className="staffAnswerSection" key={`love-${id}`}><h4>請選擇「{primary?.name || "諮詢者"}」與「{person?.name || "這位對象"}」的關係狀態</h4>
          <label>目前關係狀態<select value={text(details.relationship_status) === "分手想復合" ? "已分手" : text(details.relationship_status)} onChange={(e) => setTargetData("relationship_details", id, { ...details, relationship_status: e.target.value })}><option value="">請選擇</option>{["夫妻","已離婚","單戀中","交往中","情侶吵架","已分手","曖昧卡住","外遇對象","複雜關係","其他"].map(x => <option key={x}>{x}</option>)}</select></label>
          {details.relationship_status === "其他" && <label>其他關係狀態<input value={text(details.relationship_status_other)} onChange={(e) => setTargetData("relationship_details", id, { ...details, relationship_status_other: e.target.value })}/></label>}
      ...
      {overall && <section className="staffAnswerSection staffOverallFocusSection">
        <div className="staffAnswerSectionTitleRow"><div><h4>目前最想聚焦、最關心的具體事件</h4><small>可直接改選，與前台相同：至少 1 個、最多 3 個；勾選後下方會出現對應欄位。</small></div><b>{(extra.overall_focuses || []).length}/3</b></div>
        <div className="staffHealthChecks staffOverallFocusChecks">{Object.keys(overallFocusFieldLabels).map((focus) => {
          const selected=(extra.overall_focuses || []).includes(focus);
          return <label className={selected?"selected":""} key={focus}><input type="checkbox" checked={selected} onChange={(event) => {
            const current:string[]=extra.overall_focuses || [];
            if(event.target.checked && current.length>=3){window.alert("最多可選擇 3 個事件");return;}
            const next=event.target.checked?[...current,focus]:current.filter((item:string)=>item!==focus);
            setExtra("overall_focuses",next);
          }}/><span>{focus}</span></label>;
        })}</div>
        {(extra.overall_focuses || []).map((focus: string) => { const saved = extra.overall_focus_details?.[focus] || {}; const fields = overallFocusFieldLabels[focus] || Object.keys(saved); return <div className="staffNestedFields staffOverallFocusDetail" key={focus}><h4>{focus}</h4>{fields.map((field) => <label key={field}>{field}<textarea value={text(saved[field])} onChange={(event) => setExtra("overall_focus_details", {...(extra.overall_focus_details || {}), [focus]: {...saved, [field]: event.target.value}})}/></label>)}</div>})}
      </section>}
      ...
      {health && <section className="staffAnswerSection"><h4>當前關注的健康問題（可複選）</h4><div className="staffHealthChecks">{["睡眠／精神","腸胃／消化","心血管／頭痛","骨骼／關節","呼吸／過敏",profiles.find((p:any)=>p.id===ids[0])?.gender==="男"?"泌尿系統":"婦科／備孕","情緒／壓力"].map((name) => <label className={(extra.health_concerns || []).includes(name)?"selected":""} key={name}><input type="checkbox" checked={(extra.health_concerns || []).includes(name)} onChange={(event) => setExtra("health_concerns", event.target.checked ? [...(extra.health_concerns || []), name] : (extra.health_concerns || []).filter((item: string) => item !== name))}/><span>{name}</span></label>)}</div>{select("近期是否有手術或重大治療規劃？", "major_treatment_planned", ["是","否"])}{extra.major_treatment_planned === "是" && <>{select("想瞭解的問題", "treatment_question", ["目前的醫療團隊／主治醫師跟我有緣嗎？過程會不會順利？","手術後的恢復期運勢如何？能不能如期康復？","這段治療期間，命盤上有沒有什麼特別要注意的卡關或併發風險？","其他（自填）"])}{extra.treatment_question === "其他（自填）" && area("其他想瞭解的問題", "treatment_question_other")}</>}{area("備註", "health_notes", "若已有預定的『醫療機構／醫院名稱』與『主治醫師』，可以一併提供（例：台大醫院／陳宣醫師）。")}</section>}
...
function TargetQuestions({ primary, profile, kind, questions, update }: any) {
  const list: string[] = Array.from({length:3},(_,index)=>questions[index]||"");
